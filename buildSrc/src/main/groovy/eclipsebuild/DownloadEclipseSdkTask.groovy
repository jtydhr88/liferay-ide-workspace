...
class DownloadEclipseSdkTask extends DefaultTask {
    ...
    private void downloadEclipseSdkUnprotected(Project project) {
        // won't re-download the sdk if the sdk was unzipped

        File unzipDir = new File(targetDir, 'eclipse')

        File unzipDirMacos = new File(targetDir, 'Eclipse.app')

        if ((unzipDir.exists() && unzipDir.listFiles().length > 0) || (unzipDirMacos.exists() && unzipDirMacos.listFiles().length > 0)) {
            return
        }
