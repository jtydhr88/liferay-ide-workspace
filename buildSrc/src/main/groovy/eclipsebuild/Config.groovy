...
class Config {
    ...
    File getTargetPlatformsDir() {
        ...
        project.hasProperty('targetPlatformsDir') ?
                ...
                new File(System.getProperty('user.home'), '.liferay-ide/eclipse/targetPlatforms')
    }
    ...
    File getJarProcessorJar() {
        String pluginPath = '/eclipse/plugins';

        if (Constants.getOs().equals("macosx")) {
            pluginPath = '/eclipse/Eclipse.app/Contents/Eclipse/plugins';
        }

        new File(eclipseSdkDir.path, pluginPath).listFiles().find { it.name.startsWith('org.eclipse.equinox.p2.jarprocessor_') }
    }
}