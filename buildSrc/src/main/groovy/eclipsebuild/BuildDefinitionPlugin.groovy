 ...
 * {@link BundlePlugin}, {@link FeaturePlugin}, {@link UpdateSitePlugin}.
 * <p/>
...
class BuildDefinitionPlugin implements Plugin<Project> {
    ...
    static void assembleTargetPlatformUnprotected(Project project, Config config) {
        ...
        // TODO charles no need to totally re-download plugins
        /*
        ...
        **/
        ...
        rootNode.locations.location.each { location ->
            def url =location.repository.@location.text()

            url = new SimpleTemplateEngine().createTemplate(url).make(eclipseMirror: eclipseMirror).toString()
            updateSites.add(url.replace('\${project_loc}', 'file://' +  project.projectDir.absolutePath))
